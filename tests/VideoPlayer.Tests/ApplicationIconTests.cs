...
public class ApplicationIconTests
{
    ...
    public void Logo_is_not_placed_on_caption_or_transport()
    {
        ...
        Assert.DoesNotContain("Ieseo.ico", mainXaml, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("Assets/Ieseo", mainXaml, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("Icon=", mainXaml, StringComparison.Ordinal);

        var transportFrom = mainXaml.IndexOf("x:Name=\"TransportBar\"", StringComparison.Ordinal);
        Assert.True(transportFrom >= 0);
        var transportUntil = mainXaml.IndexOf("x:Name=\"SidebarRailColumn\"", transportFrom, StringComparison.Ordinal);
        Assert.True(transportUntil > transportFrom);
        var transport = mainXaml[transportFrom..transportUntil];
